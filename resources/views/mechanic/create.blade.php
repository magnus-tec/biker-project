<x-app-layout>
    ...
    <div class=" w-3/4 mx-auto py-8">
        <form id="formMechanic">
            @csrf
            <h5 class="text-lg font-semibold mb-4">Datos de Mecanico</h5>
...
<script>
    let formMechanic = document.getElementById('formMechanic');
    formMechanic.addEventListener('submit', function(e) {
        e.preventDefault();
        let formData = new FormData(formMechanic);

        fetch('{{ route('mechanics.store') }}', {
                method: 'POST',
                body: formData
            })
            .then(response => {
