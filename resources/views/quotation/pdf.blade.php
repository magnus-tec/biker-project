...
<head>
    <meta charset="UTF-8">
    <title>Pedido #{{ $quotation->id }}</title>
    <style>
    ...
    <div class="container">
        ...
        <table class="info-table" width="100%" style="border: 1px solid #000; border-collapse: collapse;">
            <tr>
                ...
                <td style="text-align: left; padding: 5px; ">{{ $quotation->customer_dni }}</td>
            ...
            <tr>
                ...
                <td style="text-align: left; padding: 5px;">{{ $quotation->customer_names_surnames }}
                </td>
            ...
            <tr>
                ...
                <td style="text-align: left; padding: 5px;">
                    {{ $quotation->userRegister->name ?? 'N/A' }}
                </td>
                <th></th>
                <td></td>
            </tr>
        ...
        <table width="100%" style="border-collapse: collapse; border: 1px solid #000;margin-top: 3px;">
            ...
            <tbody>
                @if (isset($quotation->quotationItems) && is_iterable($quotation->quotationItems))
                    @php
                        $totalRows = 38;
                        $rowCount = 0;
                    @endphp
                    ...
                    @foreach ($quotation->quotationItems as $index => $quotationItem)
                        @php
                            $descripcion = $quotationItem->item->description ?? ($saleItem->item->name ?? 'N/A');
                            $cantidad = str_pad($quotationItem->quantity ?? 0, 4, '0', STR_PAD_LEFT); // Formato 0001
                            $precio = $quotationItem->unit_price ?? 0;
                            $subtotal = $cantidad * $precio;
                            $unidad = $quotationItem->item->unit->name ?? 'N/A';
                            $rowCount++;
                        @endphp
                        <tr>
        ...
        <table class="totals">
            <tr>
                ...
                <td>S/ {{ number_format($quotation->total_price - $quotation->igv, 2) }}</td>
            </tr>
            <tr>
                <td>IGV:</td>
                <td>S/ {{ number_format($quotation->igv, 2) }}</td>
            </tr>
            <tr>
                <td><strong>Total a Pagar:</strong></td>
                <td><strong>S/ {{ number_format($quotation->total_price, 2) }}</strong></td>
            </tr>
        </table>
        <!-- Pie de Página -->
    </div>
