    ...
    <div class="container mx-auto p-2 text-sm">
        <div class="grid grid-cols-3 gap-6">
            ...
            <div class="col-span-2 bg-white p-6 rounded-lg shadow">
                <input type="hidden" id="quotationId" value="{{ $mayorista->id }}">
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", async function() {
        ...
        try {
            let response = await fetch(`{{ route('wholesalers.detallesWholesaler', ':id') }}`.replace(':id',
                quotationId));
            if (!response.ok) throw new Error("Error al obtener la cotización");

            let responseWholesaler = await response.json();
            console.log("log", responseWholesaler)

            // Llenar los campos con los datos de la cotización
            document.getElementById("nombres_apellidos").value = responseWholesaler.mayorista
                .customer_names_surnames;
            document.getElementById("dni_personal").value = responseWholesaler.mayorista.customer_dni;
            document.getElementById("direccion").value = responseWholesaler.mayorista.customer_address;
            console.log('responseWholesaler', responseWholesaler);
            document.getElementById("datos_mecanico").value = responseWholesaler.mayorista.mechanic.name +
                ' ' + responseWholesaler.mayorista.mechanic.apellidos;
            document.getElementById("mechanics_id").value = responseWholesaler.mayorista.mechanic.id;

            let regionId = responseWholesaler.mayorista.district.province.region.id;

            document.getElementById("regions_id").value = regionId;
            let provinceId = responseWholesaler.mayorista.district.province.id;
            let districtId = responseWholesaler.mayorista.district.id;
            ...
            quotationItems = responseWholesaler.mayorista.wholesaler_items
            ...
            services = responseWholesaler.mayorista.wholesaler_items
            ...
            updateTotalAmount();
            updateInformationCalculos();
            console.log("services", responseWholesaler.mayorista.wholesaler_items);
        } catch (error) {
    ...
    document.getElementById("save").addEventListener("click", async () => {
        try {
            ...
            const response = await fetch('{{ route('wholesalers.update', ':id') }}'.replace(':id',
                quotationId), {
                method: 'PUT',
                headers: {
