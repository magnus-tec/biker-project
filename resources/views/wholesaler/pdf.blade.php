...
<head>
    <meta charset="UTF-8">
    <title>Pedido #{{ $mayorista->id }}</title>
    <style>
...
<body>
    <div class="container">
        ...
        <div class="container">
            <div class="header">
                ...
                <div class="order-info">
                    <h2>Mayorista #: {{ $mayorista->id }}</h2>
                </div>
            </div>
        </div>

        <!-- Información del Cliente y Venta -->
        <table class="" width="100%" style="border: 1px solid #000; border-collapse: collapse;">
            <tr>
                <th style="text-align: right; padding: 5px; ">DNI CLIENTE :</th>
                <td style="text-align: left; padding: 5px; ">{{ $mayorista->customer_dni }}</td>
                <th style="text-align: right; padding: 5px;">FECHA Y HORA :</th>
                <td style="text-align: left; padding: 5px;">
                    {{ $mayorista->fecha_registro }}
                </td>
            </tr>
            <tr>
                <th style="text-align: right; padding: 5px;">CLIENTE :</th>
                <td style="text-align: left; padding: 5px;">{{ $mayorista->customer_names_surnames }}
                </td>
                <th style="text-align: right; padding: 5px;">VENDEDOR :</th>
                <td style="text-align: left; padding: 5px;">
                    {{ $mayorista->userRegister->name ?? 'N/A' }}
                </td>
            </tr>
        ...
        <table width="100%" style="border-collapse: collapse; border: 1px solid #000;margin-top: 3px;">
            ...
            <tbody>
                @if (isset($mayorista->wholesalerItems) && is_iterable($mayorista->wholesalerItems))
                    @php
                        $totalRows = 38;
                        $rowCount = 0;
                    @endphp

                    @foreach ($mayorista->wholesalerItems as $index => $wholesalerItem)
                        @php
                            $descripcion = $wholesalerItem->item->description ?? ($wholesalerItem->item->name ?? 'N/A');
                            $cantidad = $wholesalerItem->quantity ?? 0;
                            $precio = $wholesalerItem->unit_price ?? 0;
                            $subtotal = $cantidad * $precio;
                            $unidad = $wholesalerItem->item->unit->name ?? '-';
                            $rowCount++;
                        @endphp
                        <tr>
        ...
        <table class="totals">
            <tr>
                ...
                <td>S/ {{ number_format($mayorista->total_price - $mayorista->igv, 2) }}</td>
            </tr>
            <tr>
                <td>IGV:</td>
                <td>S/ {{ number_format($mayorista->igv, 2) }}</td>
            </tr>
            <tr>
                <td><strong>Total a Pagar:</strong></td>
                <td><strong>S/ {{ number_format($mayorista->total_price, 2) }}</strong></td>
            </tr>
        </table>
        <!-- Pie de Página -->
    </div>
