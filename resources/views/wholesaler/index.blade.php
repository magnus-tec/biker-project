<x-app-layout>
    ...
    <div class="max-w-7xl  mx-auto px-4 py-12">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-gray-800">Registro de Mayoristas</h2>
            ...
            <a href="{{ route('wholesalers.create') }}"
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden mb-5">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="tbodyWholesalers">
                </tbody>
            </table>
        </div>
    ...
    <script>
        ...
        function finAllWholesalers() {
            let desde = document.getElementById('fecha_desde').value;
            let hasta = document.getElementById('fecha_hasta').value;
            fetch(
                    `{{ route('wholesalers.filtroPorfecha') }}?fecha_desde=${encodeURIComponent(desde)}&fecha_hasta=${encodeURIComponent(hasta)}`
                )
                .then(response => response.json())
                .then(data => {
                    let tbody = document.getElementById('tbodyWholesalers');
                    tbody.innerHTML = '';
                    if (data.length > 0) {
                        data.forEach(mayorista => {
                            let row = document.createElement('tr');
                            row.innerHTML = `
                        <td class="px-3 py-1 whitespace-nowrap text-sm text-gray-900">${mayorista.code}</td>
                        <td class="px-3 py-1 whitespace-nowrap text-sm text-gray-900">${mayorista.customer_names_surnames == null ? 'Sin cliente' : mayorista.customer_names_surnames}</td> 
                        <td class="px-3 py-1 whitespace-nowrap text-sm text-gray-900">${mayorista.customer_dni}</td>
        ...
        async function verDetalles(quotationId) {
            try {
                let url = `{{ route('wholesalers.detallesWholesaler', ':id') }}`.replace(':id', quotationId);
                ...
                document.getElementById("ventaCliente").textContent = data.mayorista.customer_names_surnames;
                document.getElementById("ventaVendedor").textContent = data.mayorista.user_register.name;
                document.getElementById("ventaDni").textContent = data.mayorista.customer_dni;
                document.getElementById("ventaFecha").textContent = data.mayorista.fecha_registro;
                // document.getElementById("ventaTotal").textContent = parseFloat(data.sale.total_price).toFixed(2);
                document.getElementById('ventaSubTotal').value = parseFloat(data.mayorista.total_price - data.mayorista
                        .igv)
                    .toFixed(2);
                document.getElementById('ventaIGV').value = data.mayorista.igv;
                document.getElementById('ventaTotal').value = parseFloat(data.mayorista.total_price).toFixed(2);
                ...
                data.mayorista.wholesaler_items.forEach(item => {
        ...
        async function editWholesaler(quotationId) {
            let url = `{{ route('wholesalers.edit', ':id') }}`.replace(':id', quotationId);
            window.location.href = url;
        }
        async function deleteWholesaler(quotationId) {
            ...
            try {
                let url = `{{ route('wholesalers.destroy', ':id') }}`.replace(':id', quotationId);
                let response = await fetch(url, {
                    method: 'DELETE',
                    headers: {
        ...
        async function generarPDF(quotationId) {
            try {
                let url = `{{ route('wholesalers.pdf', ':id') }}`.replace(':id', quotationId);
